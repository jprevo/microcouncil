...
const CHEAT_SHEET_URL = 'https://raw.githubusercontent.com/ikatyang/emoji-cheat-sheet/master/README.md';
...
async function fetchText(url) {
  const response = await fetch(url, { headers: { 'user-agent': 'microcouncil-build' } });
...
function parseShortcodes(readme) {
  ...
  for (const line of readme.split('\n')) {
    ...
    for (const [, code] of line.matchAll(/`:([a-z0-9_+-]+):`/g)) codes.push(code);
  }
  return codes;
}
...
const [readme, apiBody] = await Promise.all([fetchText(CHEAT_SHEET_URL), fetchText(GITHUB_EMOJI_API)]);
...
if (count < 1000) throw new Error(`only ${count} emoji parsed: the upstream format probably changed`);
